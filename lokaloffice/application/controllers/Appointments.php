...
class Appointments extends CI_Controller {
    ...
    
    {
        try
        {
            ...
            if (count($records) == 0)
            {
                throw new Exception('Não foram encontrados registros para este código de agendamento.');
            }
            ...
            //Verifica se o cancelamento está no prazo
            date_default_timezone_set('America/Sao_Paulo');
            //display the converted time
            $datetime_limit = date('Y-m-d H:i',strtotime('-8 hour',strtotime($start_datetime)));
    		
            //Se a data limite for maior que a data de cancelamento transfere as horas para o banco de horas.
    
            if(strtotime($datetime_limit) > strtotime('now')){ //
    
              $id = $appointment['id_users_customer'];                
             
              //Se existir ID traz quantidade de horas
              if(isset($id)){
                  $hours = $this->customers_model->get_value('hours', $id);  
                  $first_name = $this->customers_model->get_value('first_name', $id); 
                  $last_name = $this->customers_model->get_value('last_name', $id); 
                  $email = $this->customers_model->get_value('email', $id);    
                  $phone_number = $this->customers_model->get_value('phone_number', $id); 
                
              }
    
              //Soma horas canceldas com o banco de horas
              $appointmentHours=0;
              
              if($appointment['id_services']==1){
                $appointmentHours=1;
              }
              if($appointment['id_services']==2){
                $appointmentHours=1.5;
              }
              if($appointment['id_services']==3){
                $appointmentHours=2;
              }
              if($appointment['id_services']==4){
                $appointmentHours=3;
              }

              $hours=$hours+$appointmentHours;
            ...
            try
            {
                ...
                if ($send_provider === TRUE)
                {
                    $email->sendDeleteAppointment($appointment, $provider,
                        $service, $customer, $company_settings, new Email($company_email),
        ...
        if(strtotime($datetime_limit) > strtotime('now')){
        $view = [
            'message_title' => $this->lang->line('appointment_cancelled_before_limit_title'),
            'message_text' => $this->lang->line('appointment_cancelled_before_limit'),
            'message_icon' => base_url('assets/img/success.png')
        ];
        }else{

            $view = [
                'message_title' => $this->lang->line('appointment_cancelled_after_limit_title'),
                'message_text' => $this->lang->line('appointment_cancelled_after_limit'),
                'message_icon' => base_url('assets/img/success.png')
            ];
        }
        ...
        try
        {
            ...
            if ($post_data['manage_mode'] === TRUE){
                ...
                if(strtotime($datetime_limit) < strtotime('now')){
                    ...
                    // appointment_exceed_limit to make changes. Display
                    // a message to the customer.

                    throw new Exception($this->lang->line('appointment_exceed_limit'));
                }
            }
            ...
            try
            {
                ...
                if ($send_provider === TRUE)
                {
                    $email->sendAppointmentDetails($appointment, $provider,
                        ...
                        $provider_message, $provider_link, new Email($company_email), new Text($ics_stream));
                }
            }
            catch (Exception $exc)
            {
    ...
    public function ajax_get_hours()
    {
        try
        {
            ...
            if ($this->customers_model->exists($customer))
            {
                $id = $this->customers_model->find_record_id($customer);                
            }else{
            ...
            if(isset($id)){
                ...
                $email = $this->customers_model->get_value('email', $id);    
                $phone_number = $this->customers_model->get_value('phone_number', $id); 
                $address = $this->customers_model->get_value('address', $id); 
                $city = $this->customers_model->get_value('city', $id);
                $zip_code = $this->customers_model->get_value('zip_code', $id);    
            }     
    ...
    public function ajax_get_customer()
    {
        ...
        try
        {
            ...
            if(isset($id)){
                ...
                $address = $this->customers_model->get_value('address', $id); 
                $city = $this->customers_model->get_value('city', $id);
                //$state = $this->customers_model->get_value('state', $id);
                $zip_code = $this->customers_model->get_value('zip_code', $id);    
            }     
    ...
    public function pagSeguroApi(){
        ...
        if($_POST['hour']=="1"){
            ...
            $data['itemDescription1'] = 'Aluguel 1 hora';
            $data['itemAmount1'] = '30.00';

        } elseif($_POST['hour']=="1.5"){
    
            $data['itemId1'] = '2';
            $data['itemQuantity1'] = '1';
            $data['itemDescription1'] = 'Aluguel 1 hora e 30 minutos';
            $data['itemAmount1'] = '45.00';
       
        } elseif($_POST['hour']=="2"){
    
            $data['itemId1'] = '3';
            $data['itemQuantity1'] = '1';
            $data['itemDescription1'] = 'Aluguel 2 horas';
            ...
            $data['itemId1'] = '4';
            $data['itemQuantity1'] = '1';
            $data['itemDescription1'] = 'Aluguel 3 horas';
            $data['itemAmount1'] = '90.00';
       
        }

        $data['reference'] = $_POST['appointmentId'];

        $data['acceptPaymentMethodGroup']=('CREDIT_CARD');
        //$data['excludePaymentMethodGroup'] = ('BOLETO');
     
        //$data['excludePaymentMethodGroup']=["BOLETO", "DEPOSIT"];
        
        //$url = 'https://ws.sandbox.pagseguro.uol.com.br/v2/checkout'; //URL Sandbox
        ...
        $pagseguro['id_pagseguro'] = $xml->code;
        $pagseguro['value'] = 0;
        $pagseguro['email'] = $_POST['email'];
        $pagseguro['appointmentId']=$_POST['appointmentId'];
        

        // $this->load->model('payments_model');
        // $paymentId= $this->payments_model->add($pagseguro);
        ...
        $sql = "INSERT into ea_payments (id_pagseguro, value, email, id_appointment, start_datetime, end_datetime) VALUES 
                ('".$pagseguro['id_pagseguro']."',
                '".$pagseguro['value']."',
                '".$pagseguro['email']."',
                '".$pagseguro['appointmentId']."',
        ...
        $sql2 = "UPDATE ea_appointments SET start_datetime ='1010-10-10 10:00:00', end_datetime ='1010-10-10 11:00:00'
                 WHERE id = '" . $pagseguro['appointmentId'] . "'";
        ...
        public function removeAppointmentApi(){
            ...
            $sql = "DELETE FROM ea_appointments 
            WHERE id = '" . $_POST['appointmentId'] . "'";
